...
**Velocity curve**:
How hard a key is struck against how loud the instrument plays it, set by the volume the lightest strike earns and the shape of the path from there to full. Calibrated by ear. Distinct from the keyboard volume, which trims the result, and from the velocity offset, which calibrates grading alone.
_Avoid_: Velocity sensitivity, dynamics curve
