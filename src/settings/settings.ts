// The global settings, as the window holds them: one copy in memory, read from the Rust side once
...
/** The fields a knob map names, read off the settings: the reverse of `knobDefaults`. */
export function knobValues<M extends Partial<Record<SettingKey, string>>>(
  knobs: M,
): { [K in keyof M as M[K] & string]: Settings[K & SettingKey] } {
  return Object.fromEntries(
    Object.entries(knobs).map(([key, field]) => [field, values[key as SettingKey]]),
  ) as { [K in keyof M as M[K] & string]: Settings[K & SettingKey] };
}
...
let values: Settings = { ...SETTING_DEFAULTS };
const perKey = new Map<SettingKey, Set<() => void>>();
const everyKey = new Set<() => void>();
...
/**
 * The stored values this shape of the app can use. A value of another type than its default is
 * from an older shape of the setting, so the default stands in for it; a null default accepts
 * anything.
 */
...
/** One setting as it stands, for a reader that is not a React component. */
export function setting<K extends SettingKey>(key: K): Settings[K] {
  return values[key];
}

/** Calls `listen` whenever one setting changes, until the returned function is called. */
export function subscribe(key: SettingKey, listen: () => void): () => void {
  const set = perKey.get(key) ?? new Set<() => void>();
  perKey.set(key, set);
  set.add(listen);
  return () => {
    set.delete(listen);
...
/** One setting, re-rendering the component on a change of that key alone. */
export function useSetting<K extends SettingKey>(key: K): Settings[K] {
  return useSyncExternalStore(
    (listen) => subscribe(key, listen),
    () => values[key],
  );
}

/** Every setting at once, for the settings panel, which draws them all. */
export function useSettings(): Settings {
  return useSyncExternalStore(subscribeAll, () => values);
}

/**
 * Writes one setting: memory first, so what the user just did is on screen at once, then the Rust
 * side. A value the sound engine refuses puts the old one back and is answered with the reason;
 * an empty string means the value stands.
 */
export async function set<K extends SettingKey>(key: K, value: Settings[K]): Promise<string> {
...
function hold<K extends SettingKey>(key: K, value: Settings[K]): void {
  values = { ...values, [key]: value };
  notify(key);
}

function notify(key: SettingKey): void {
  for (const listen of perKey.get(key) ?? []) listen();
  for (const listen of everyKey) listen();
}

function subscribeAll(listen: () => void): () => void {
  everyKey.add(listen);
  return () => {
    everyKey.delete(listen);
  };
}
